...
void UInv_SpatialInventoryWidget::OnItemHovered(UInv_InventoryItem* Item)
{
	...
	TimerDelegate.BindLambda([this, &Manifest, Item]()
	{
		...
		EquippedTimerDelegate.BindUObject(this, &ThisClass::ShowEquippedItemDescription, Item);
		GetOwningPlayer()->GetWorldTimerManager().SetTimer(EquippedDescriptionTimer, EquippedTimerDelegate, EquippedDescriptionTimerDelay, false);
	...
	const FGameplayTag& EquipmentTypeTag) const
{
	...
	const UInv_HoverItemWidget* HoverItem = GetHoverItem();
...
void UInv_SpatialInventoryWidget::ShowEquippedItemDescription(UInv_InventoryItem* Item)
{
	const auto& Manifest = Item->GetItemManifest();
	...
	auto EquippedGridSlot = EquippedGridSlots.FindByPredicate([Item](const UInv_EquippedGridSlotWidget* GridSlot)
	{
		return GridSlot->GetInventoryItem() == Item;
	});
	if (EquippedGridSlot != nullptr) return; // 鼠标悬停的物品已装备，我们已显示其物品描述

	// 未装备，请寻找已装备同装备类型的物品
	auto FoundEquippedSlot = EquippedGridSlots.FindByPredicate([HoveredEquipmentType](const UInv_EquippedGridSlotWidget* GridSlot)
	{
		return GridSlot->GetInventoryItem().IsValid() && 
	   GridSlot->GetInventoryItem()->GetItemManifest().GetFragmentOfType<FInv_EquipmentFragment>() &&
	   GridSlot->GetInventoryItem()->GetItemManifest().GetFragmentOfType<FInv_EquipmentFragment>()->GetEquipmentType() == HoveredEquipmentType;
	});
	const UInv_EquippedGridSlotWidget* EquippedSlot = FoundEquippedSlot ? *FoundEquippedSlot : nullptr;
	if (!IsValid(EquippedSlot)) return; // 没有装备相同装备类型的物品
...
UInv_ItemDescription* UInv_SpatialInventoryWidget::GetEquippedItemDescription()
{
...
void UInv_SpatialInventoryWidget::SetEquippedItemDescriptionSizeAndPosition(UInv_ItemDescription* Description,
